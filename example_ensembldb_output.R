#!/usr/bin/env Rscript
# Example demonstrating the extract_gene_attributes_ensembldb.R script functionality
# This creates sample output showing what the script produces when run with a local database
# No database required - creates example data for demonstration purposes
...
output_file <- "example_gene_attributes_ensembldb.txt"
write.table(
  sample_data,
  file = output_file,
  sep = "|",
...
cat("\nThis demonstrates the format of the output from extract_gene_attributes_ensembldb.R\n")
cat("The format is identical to extract_gene_attributes.R (biomaRt version)\n")
...
cat("\nKey differences from biomaRt version:\n")
cat("  - Uses local EnsDb databases (offline)\n")
cat("  - Faster query performance\n")
cat("  - Fixed Ensembl version for reproducibility\n")
cat("  - Output format is identical\n")
