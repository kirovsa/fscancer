...
GetOptions(
    ...
    'stats=s'       => \$stats_file,
) or die "Usage: $0 [--metadata FILE] [--verbose] [--include-model] [--stats FILE]\n";
...
if ($stats_fh) {
    ...
    print $stats_fh "#\n";
    print $stats_fh "project\trows_accepted\trows_rejected\tsamples_accepted\tsamples_rejected\n";
    ...
    foreach my $projname (sort keys %project_stats) {
        ...
        print $stats_fh "$projname\t$stats->{accepted}\t$stats->{rejected}\t$samples_accepted_count\t$samples_rejected_count\n";
